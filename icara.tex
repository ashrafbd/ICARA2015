...
\notedme{This section is too long. You should be onto section 2 by page 2 or 3, I'd say. Where's the background section? Seems it starts half-way through current section 1...}
Wireless Sensor Networks (WSN) have attracted many robotics researchers\notedme{evidence?} because of the potentials for many problem solving applications, such as robot localisation, path finding, sensing, mapping. 
The wireless sensors and robotics can be coined\notedme{?} to offer great advantages in many fields like ambient assisted living, environmental monitoring, vine pruning, and so on.\notedme{needs citations}
...
They may necessitate reprogramming\notedme{not grammatically correct} after deployment to accommodate alteration in the environment, sensing applications, or scientific requirements~\cite{ISI:000253439700120}.
...
Combining the capabilities of WSN and robots is appealing. %are therefore very conducive in a unified fashion.


Reprogramming of the sensors is one of the most important management tasks.
It is not easy to round up all the deployed sensors in order to reprogramme\notedme{I'd probaby use `reprogram' throughout... even if it is an American English form} them. 
...
From network point of view, the update process across the whole WSN contributes to a significant increase in traffic that an adversary may exploit to  mask his actions. 
...
Despite the need for security research within WSNs, only few previous studies have considered IDS approach in recent years.\notedme{Why is there a need but not action? This seems odd...}
